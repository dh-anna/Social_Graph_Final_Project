...
def create_random_graph(source_graph, actor_directors_dict):
    G = nx.DiGraph()
    nodes = list(source_graph.nodes())
    G.add_nodes_from(nodes)
    n_simulations=len(actor_directors_dict.keys())
    lengths = [len(v) for v in actor_directors_dict.values()]
    node_counts = 0
    for _ in range(n_simulations):
        current_node = random.choice(nodes)

        for _ in range(lengths[node_counts]):
            target_node = random.choice(nodes)

            if G.has_edge(current_node, target_node):
                G[current_node][target_node]['weight'] += 1
            else:
...
def compare_to_random_baseline(observed_G, random_G, threshold=2.0):
    results = []

    for u, v, data in observed_G.edges(data=True):
        observed_weight = data.get('weight', 1)

        if random_G.has_edge(u, v):
            expected_weight = random_G[u][v].get('weight', 1)
...
def print_findings(results, top_n=10):
    print(f"Top {min(top_n, len(results))} edges with higher-than-expected weights:\n")

    for r in results[:top_n]:
        print(f"The link weight between {r['source']} and {r['target']} was "
              f"{r['ratio']:.1f} times higher than expected if movement were random.")
        print(f"  (Observed: {r['observed']:.2f}, Expected: {r['expected']:.2f})")
        print()
...
def visualize_actor_transitions(transitions, total_actors, actor_min_films):
    ...
    fig.update_layout(
        ...
        margin=dict(l=20, r=20, t=200, b=100),  # Add margins (left, right, top, bottom)
    )
    fig.write_image('actor_career_transitions_sankey.png', width=1000, height=600)
